/* NetworkFlow.java
   ...
   Starter cvode by B. Bird - 07/05/2014

   @Chenkai Hao - Dec 2014
*/
...
public class NetworkFlow{
	...
	/* MaxFlow(G)
	   ...
	   across the edge (i,j).
	*/
	static int[][] MaxFlow(int[][] G){	//ford-fulkerson algorithm
		int numVerts = G.length;
		
		boolean visited[] = new boolean[numVerts];
		
		//create a residual graph and modify on it
		...
		while(dfs(G,residual, source, dest, visited, pathFlow)>0){
	...
	public static int dfs(int[][] graph, int[][] residual, int s, int t, boolean[] visited, int min){
		int n = graph.length;
		if(s==t){
			return min;
		}
		...
		for(int i=0; i<n; i++){
			if(!visited[i] && graph[s][i]>0){
				int minn = Math.min(min, graph[s][i]);
				int v = dfs(graph, residual, i, t, visited, minn);
				if(v>0){
					graph[s][i] = graph[s][i]-v;
					residual[s][i] = residual[s][i]+v;
					graph[i][s] = graph[i][s]+v;
					residual[i][s] = residual[i][s]-v;
					return v;
				}
			}
		}
